...
- The current Linux CLI now also exposes `doctor`, `capture-atpcap`, `summarize-atpcap`, and `replay-atpcap`, so packaging checks and offline diagnostics are no longer just planned.
...
- The repo now also carries first install artifacts under `packaging/linux/`: a checked-in `udev` rule template and an install script for published Linux builds.
...
- Current Linux `.atpcap` version 3 capture preserves normalized contact frames but does not yet encode physical click state for replay.
