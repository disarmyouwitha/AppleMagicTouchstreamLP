...
public readonly record struct AtpCapV3Meta(
    string Type,
    string Schema,
    string? CapturedAt,
    string? Platform,
...
public readonly record struct AtpCapV3Contact(
    int Id,
    float X,
    float Y,
    float Total,
...
public readonly record struct AtpCapV3Frame(
    ulong Sequence,
...
public readonly record struct AtpCapV3Compatibility(
    bool FlipY,
...
public static class AtpCapV3Payload
{
    public const uint FrameMagic = 0x33564652;
    public const byte FrameReportId = 0x52;
        ...
        bool flipY = false)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            float sourceY = flipY && float.IsFinite(source.Y) ? (1.0f - source.Y) : source.Y;
