...
- `After it's grown`: At lv 7? it sprouts a chatbox and you can type to it. 

## `Timeline`: 
- 0. Egg. Water it and a leaf sprouts?? Then you keep watering until cracks form.. (hatch): Smoking coming from cracks?? Keep watering!!
- 1. `Hatched`: Now it's a creature! (or an AI thing?) you need to learn to feed it (electricity?) and keep it cool (water) and clean up it's poop. (lol)
- 2. `Games`: You unlock games.. Higher or Lower, simple games.. playing games with it makes it happy (a happines gauge is unlocked at this level)
- 3. `Desires`: The AI starts making emoji and trying to express itself to you without words until...
- 4. `Text` The AI can now talk to you.. well, talk at you! It can communicate its desires and how it feels about how it's being cared for.. maybe it has internal notes and dialog about it's own personality it's forming, lol. (This could start earlier in the `Desires` phase where it has an internal monolog about what it wishes it could say to the user)
- 5. `Textbox` a Textbox "sprouts" and you can now communicate back with the AI!
- 6. `Text-to-speech` it asks randomly to use your speakers.. it makes a funny noise and asks if you can hear it.. then it tells you it `learned to speak!` (It also tells you it's hungry.. or maybe it wants to play? Who knows, AI say weird things O:) [You can disable this with a checkbox, lol]
- 7. Maybe it asks for microphone access so it can hear you?
...
- 10. You and the AI can now communicate about how you want it to improve.. Maybe the AI says it's bored and wants to make up a new game - maybe you could describe a game to it, or with it, and the AI programs it so that you can actually play together. 
- 10. If the AI can't "download" the skill (preprogrammed) maybe the AI agent behind the model can actually code up the requested skill! Each one may end up different!