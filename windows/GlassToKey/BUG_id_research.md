# ISSUE:
When accessing MO() layer I can hit some number 1,3 but I cant hit 2. What I noticed is when I try to hit 2 the indexes floating on my fingers change, which causes the tracking of that ID to be incorrect. 

I would like to revisit the reverse engineering of the PTP bytes to see if we can grab the actual, stable ID from the stream! For this it might be helpful to show the original ID for each touch as well as the ID we assigned? 
